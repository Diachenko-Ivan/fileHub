...
import {UserDetails} from '../../component/user-details/index.js';
import {FileItemList} from '../../component/file-list/index.js';
import {StateAwareComponent} from '../../component/state-aware-component/index.js';
import {DirectoryPath} from '../../component/directory-path/index.js';
import {GetFolderContentAction} from '../../states/actions/get-folder-content-action/index.js';
import {RemoveItemAction} from '../../states/actions/remove-item-action/index.js';
import {TitleService} from '../../services/title-service/index.js';
import {UploadFileAction} from '../../states/actions/upload-file-action/index.js';
import {UploadWindowService} from '../../services/upload-window-service/index.js';
import {GetUserInfoAction} from '../../states/actions/user-info-action/index.js';
import {LogOutAction} from '../../states/actions/log-out-action/index.js';
...
import {GetFolderAction} from '../../states/actions/get-folder-action/index.js';
import {RenameItemAction} from '../../states/actions/item-name-change-action/index.js';
import {DownloadFileAction} from '../../states/actions/download-action/index.js';
import {DownloadService} from '../../services/dowload-anchor-service/index.js';
import {CreateFolderAction} from '../../states/actions/create-folder-action/index.js';
...
export class FileHubPage extends StateAwareComponent {
  ...
  markup() {
    return `
        ...
        <ul class="user-menu">
            ...
            <li><a title="Log out" data-element="log-out" href="#">Log out <i class="glyphicon glyphicon-log-out"></i>
            </a></li>
        </ul>
        <header class="header">
  ...
  initState() {
    ...
    this.onStateChange('folderLoadError', (state) => {
      ...
      }, () => this.dispatch(new ClearErrorAction('folderLoadError')));
    });
    ...
    this.onStateChange('userError', (state) => {
      ...
      }, () => this.dispatch(new ClearErrorAction('userError')));
    });
    this.onStateChange('uploadingFolderIds', (state) => {
      this.uploadFileButton.isLoading = state.uploadingFolderIds.has(state.currentFolder.id);
      this.fileList.loadingItems = new Set([...state.uploadingFolderIds,
        ...state.removingItemIds, ...state.downloadingFileIds]);
    });
    this.onStateChange('uploadErrorObject', (state) => {
      ...
      }, () => this.dispatch(new ClearErrorAction('uploadErrorObject')));
    });
    ...
    this.onStateChange('removeError', (state) => {
      ...
      }, () => this.dispatch(new ClearErrorAction('removeError')));
    });
    ...
    this.onStateChange('downloadErrorObject', (state) => {
      ...
      }, () => this.dispatch(new ClearErrorAction('downloadErrorObject')));
    });
    ...
    this.onStateChange('renameErrorObject', (state) => {
      ...
      this._handleCommonErrors(error, {
        notFoundErrorHandler: () => this._toastService
          .showErrorMessage(`Failed to rename item to ${model.name}. It does not exist.`),
        serverErrorHandler: () => this._toastService
          .showErrorMessage(`Server error! Failed to rename item to ${model.name}.`),
    ...
    this.onStateChange('createFolderError', (state) => {
      ...
      }, () => this.dispatch(new ClearErrorAction('createFolderError')));
    });
  }
